document.addEventListener("DOMContentLoaded", function() {
  ...
  addExpenseButton.addEventListener("click", function() {
    ...
    if (description && category) {
      ...
      const expense = { description, amount, category, units: unit, unitName: unitNameValue, comment };
      let expenses = JSON.parse(localStorage.getItem("expenses") || "[]");
...
expenseTableBody.addEventListener("dblclick", function (event) {
      ...
      if (expenses[rowIndex]) {
        switch (columnIndex) {
          ...
          case editableColumns.amount:
            expenses[rowIndex].amount = parseFloat(newValue) || 0;
            break;
  ...
  exportDataButton.addEventListener("click", function() {
   ...
   exportCsvButton.addEventListener("click", function() {
    const expenses = JSON.parse(localStorage.getItem("expenses") || "[]");
    let csvContent = "Description,Amount,Category,Units,Unit Name,Comment\n";

    expenses.forEach(expense => {
      csvContent += `${expense.description},${expense.amount},${expense.category},${expense.units},${expense.unitName},${expense.comment}\n`;
    });
      ...
      reader.onload = function(e) {
        try {
          if (fileExtension === "csv") {
            ...
            // Ensure categories are set as well (you may need a similar CSV import for categories)
